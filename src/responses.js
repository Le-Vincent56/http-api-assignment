...
const success = (request, response, acceptedTypes) => {
    ...
    {
        let responseXML = createXMLResponse(serverResponse.message, serverResponse.id);
        return respond(request, response, 200, 'text/xml', responseXML);
    }

    // Respond with a success code in JSON format
    return respond(request, response, 200, 'application/json', serverResponse);
};
...
const badRequest = (request, response, params) => {
    ...
    if(!params.valid || params.valid !== 'true')
    {
        ...
        return respond(request, response, 400, 'application/json', serverResponse);
    }
...
const unauthorizedRequest = (request, response, params) => {
    ...
    if(!params.loggedIn || params.loggedIn !== 'yes')
    {
        ...
        // Return with 401
        return respond(request, response, 401, 'application/json', serverResponse);
    }
...
const forbidden = (request, response) => {
    ...
    return respond(request, response, 403, 'application/json', serverResponse);
};
...
const internal = (request, response) => {
    ...
    return respond(request, response, 500, 'application/json', serverResponse);
};
...
const notImplemented = (request, response) => {
    ...
    return respond(request, response, 501, 'application/json', serverResponse);
};
...
const notFound = (request, response) => {
  ...
  // Respond with a 404
  return respond(request, response, 404, 'application/json', serverResponse);
};
